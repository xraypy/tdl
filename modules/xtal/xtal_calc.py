...
def g_matx(cell): # calculate g matrix from real space lattice parameters
    ...
    gam = cell[5]
    Pi  = numpy.pi
    g = numpy.array([[a*a, a*b*(numpy.cos(gam*Pi/180)), a*c*(numpy.cos(bet*Pi/180))],
                    [b*a*(numpy.cos(gam*Pi/180)), b*b, b*c*(numpy.cos(alp*Pi/180))],
                    [c*a*(numpy.cos(bet*Pi/180)), c*b*(numpy.cos(alp*Pi/180)), c*c]])
    return g
...
def rlat(cell): # calculate reciprocal space lattice parameters
    ...
    a_r = numpy.sqrt(g_inv[0,0])
    b_r = numpy.sqrt(g_inv[1,1])
    c_r = numpy.sqrt(g_inv[2,2])
    Pi  = numpy.pi
    alp_r = (180/Pi)*numpy.arccos(g_inv[1,2]/(b_r*c_r))
    bet_r = (180/Pi)*numpy.arccos(g_inv[0,2]/(a_r*c_r))
    gam_r = (180/Pi)*numpy.arccos(g_inv[0,1]/(a_r*b_r))
...
def dot_product(u,v,cell):# calculate dot product of two vectors in basis defined by cell
    g = g_matx(cell)
    x = numpy.matrixmultiply(g,numpy.transpose(v)) 
    dot_product = numpy.matrixmultiply(u,x) # dot product = u*g*v', from Sands.
    return dot_product

##########################################################################
def vector_mag(v,cell): # calculate magnitude of a vector in basis defined by cell
    mag   = dot_product(v,v,cell)
    v_mag = numpy.sqrt(mag)
    return v_mag
...
def vector_angle(u,v,cell): # calculate angle between two vectors in basis defined by cell
    ...
    Pi = numpy.pi
    v_angle = (180/Pi)*numpy.arccos(x/(u_mag*v_mag))
    return v_angle
...
def calc_d(hkl,cell): # calculate vector d in bulk real space basis which has a magnitude of d_spacing
    g = g_matx(cell)
    g_inv = numpy.linalg.inverse(g)
    hkl = numpy.transpose(hkl)
    d_spc = d_spacing(hkl,cell)
    d = numpy.array(numpy.matrixmultiply(g_inv,hkl))
    d = d_spc*d_spc*d
    return d
...
def basis_transform_cart(cell): #calculate Gc and Fc for transformations to cartesian basis
    ...
    Gc = numpy.array([[1/a, 0 , 0],
                     [-1/(a*numpy.tan(gam*Pi/180)), 1/(b*numpy.sin(gam*Pi/180)), 0],
                     [a_r*numpy.cos(bet_r*Pi/180), b_r*numpy.cos(alp_r*Pi/180), c_r]])
    # F transforms coordinates to cartesian
    # F = inverse(G')
    Fc = numpy.array([[a, b*numpy.cos(gam*Pi/180), c*numpy.cos(bet*Pi/180)],
                     [0, b*numpy.sin(gam*Pi/180), -1*c*numpy.sin(bet*Pi/180)*numpy.cos(alp_r*Pi/180)],
                     [0, 0, 1/c_r]])
    return Fc
...
def transform_to_cart(u,v,w,cell): # transform u,v,w to cartesian
    ...
    uvw = [u, v, w]
    uvw_xyz = numpy.matrixmultiply(Fc, numpy.transpose(uvw))
    xyz = numpy.transpose(uvw_xyz1)
    return xyz
...
def cart_to_cell(x,y,z,cell): # transform x,y,z to basis defined by cell
    Fc = basis_transform_cart(cell)
    xyz = [x, y, z]
    xyz_uvw = numpy.matrixmultiply((numpy.linalg.inverse(Fc)),numpy.transpose(xyz))
    uvw = numpy.transpose(xyz_uvw)
    return uvw

##########################################################################
def trans_hexa_to_rhombo(Va): # transfrom any vector from hexa to rhombo system
    MM = numpy.array([[0.6667, 0.3333, 0.3333],
                      [-0.3333, 0.3333, 0.3333],
                      [-0.3333, -0.6667, 0.3333]])
    NN = numpy.linalg.inv(numpy.transpose(MM))
    V_rh = numpy.matrixmultiply(NN,numpy.transpose(Va))
    return V_rh
...
def trans_rhombo_to_hexa(Va): # transfrom any vector from rhombo to hexa system
    MM = numpy.array([[0.6667, 0.3333, 0.3333],
                      ...
                      [-0.3333, -0.6667, 0.3333]])
    NN = numpy.linalg.inv(numpy.transpose(MM))
    NN_inv = numpy.linalg.inv(NN)
    V_hx = numpy.matrixmultiply(NN_inv,numpy.transpose(Va))
    return V_hx
...
def basis_transformation_matrix(Va,Vb,Vc): # transform to basis defined by Va, Vb and Vc
    ...
    #        xcs   ycs  zcs
    #
    M = numpy.array([Va,Vb,Vc])
    # G does the opposite of M
    G = numpy.linalg.inv(M)
    ...
    F = numpy.linalg.inv(numpy.transpose(M))
    # N does opposite of F
    N = numpy.linalg.inv(F)
    MGFN = numpy.array([M,G,F,N])
    return MGFN
...
def calc_surf_cell(F,bulk_name,cell): # calculate the big surface cell
    ...
    for j in range(0,len(el_name)):
        v_tmp = numpy.array([xyz[j,0],xyz[j,1],xyz[j,2]])
        v = numpy.matrixmultiply(F,numpy.transpose(v_tmp))
        xyz[j,0] = v[0]
        xyz[j,1] = v[1]
        xyz[j,2] = v[2]
