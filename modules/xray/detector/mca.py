    ...
    * self.two_theta    = 0.    # 2-theta of this Mca for energy-dispersive diffraction
    ...
    def __repr__(self):
        ...
        if self.input_counts >= 0:
            ...
            lout = lout + "    >OCR     = %6.1f, ICR     = -1.\n" % (self.total_counts / self.live_time)
        try:
            icr_calc = "%6.1f" %  self.icr_calc
        except:
            icr_calc = "%s" %  str(self.icr_calc)
        lout = lout + "    >Tau     = %6.5g, ICR_Calc= %s\n" %  (self.tau, icr_calc)
        lout = lout + "    >Correct = %6.5f, Deadtime =%5.2f%%\n" %  (self.cor_factor, 100.0*(1 - 1/self.cor_factor))
        return lout
    ...
    def _calc_correction(self):
        ...
        if self.tau >= 0:
            icr = deadtime.calc_icr(ocr,self.tau)
            #if icr == None: icr = 0
            self.icr_calc = icr
