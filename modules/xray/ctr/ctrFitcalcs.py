...
class Fitting_Rod:
    ...
    def calcF(self,sig_water,Scale,specScale,beta,cell,bulk,surface,g_inv,NLayers,database, use_bulk_water):
        ...
        self.difference = (Num.log(self.F) - Num.log(self.Fcalc))**2
...
def calc_CTRs(parameter,param_usage, dat, cell, bulk_tmp, surface_tmp, NLayers, database, g_inv, Rod_weight, rigid_bodies, use_bulk_water):
    ...
    for x in dat:
        x.calcF(sig_water,Scale,specScale,beta,cell,bulk_new,surface_new,g_inv,NLayers,database, use_bulk_water)
...
def simulated_annealing01(dat, cell, NLayers, bulk, surface, database, Rod_weight, sim_an_params, parameter, param_usage, plot_RMS_track, rigid_bodies, use_bulk_water):
    ...
    print '\n####################################################\n'
    print 'the best fit RMS error is: '+str(RMS_best)+'\n'
    print '*************************************\n'
...
def plot_rods(dat, plot_dims, plot_bulk, plot_surf, plot_rough,plot_water, RMS):
    ...
    clf()
    fig1.suptitle('RMS = '+str(round(RMS,7)), fontsize=20)
    for i in range(len(dat)):
