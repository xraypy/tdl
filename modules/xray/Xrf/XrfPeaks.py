#######################################################################
"""
Fits a spectrum to a set of Gaussian peaks.  
Author: Mark Rivers, GSECARS
...
#import mpfit
from mpfit import nmpfit as mpfit
...
class XrfSpectrum:
    ...
    Fields:
        ...
        self.bgr                   = None      # Background model, see XrfBgr module
    ...
    ##########################################################################################
    def init(self,data=None,chans=None,params={},guess=False,bgr_model='Default'):
        """
        ...
        if bgr_par != None:
            if bgr_model == 'Default':
                self.bgr = XrfBgr.Background(bgr_par)
    ...
    def fit(self,guess=True,opt_bgr=True, quiet=1):
        ...
        Fit the data
        """
        if opt_bgr == False and self.bgr != None:
            bgr_model = self.bgr
            self.bgr  = None
            data      = copy.copy(self.data)
            self.data = self.data - bgr_model.calc(data,slope=self.energy_slope)
        else:
            bgr_model = None
