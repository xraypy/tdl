...
def med_copy_rois(med, source_mca=0):
    ...
    rois  = med.mcas[source_mca].rois
    off   = med.mcas[source_mca].offset
    slope = med.mcas[source_mca].slope
    quad  = med.mcas[source_mca].quad
    clip  = len(med.mcas[source_mca].data)
    ...
    for j in range(med.n_detectors):
        off   = med.mcas[j].offset
        slope = med.mcas[j].slope
        quad  = med.mcas[j].quad
        clip  = len(med.mcas[j].data)
        med.mca[j].rois = []
